...
struct LocalTokenLedger: Equatable {
    ...
    func totals(for instance: String, on day: Date, calendar: Calendar = .current) -> Totals? {
        days[instance]?[calendar.startOfDay(for: day)]
    }

    func summary(for instance: String, now: Date, calendar: Calendar = .current) -> Summary? {
        guard days[instance] != nil || last[instance] != nil else { return nil }
        return Summary(today: totals(for: instance, on: now, calendar: calendar) ?? Totals(),
                       last: last[instance])
    }

    /// Everything logged today across every instance, for Settings.
    func totalsToday(now: Date, calendar: Calendar = .current) -> Totals {

...
final class LocalTokenLedgerTests: XCTestCase {
    ...
    func testTotalsAreKeptPerInstanceAndPerLocalDay() throws {
        ...
        XCTAssertNil(ledger.summary(for: "qwen", now: morning, calendar: calendar))
        ledger.record(prediction(at: yesterday, input: 1000, output: 50), calendar: calendar)
        ledger.record(prediction(at: morning, input: 100, output: 300, reasoning: 200, drafts: (492, 176)), calendar: calendar)
        ledger.record(prediction(at: night, input: 50, output: 100, reasoning: nil, drafts: (8, 8)), calendar: calendar)
        ledger.record(prediction("other", at: night, input: 7, output: 7), calendar: calendar)

        let summary = try XCTUnwrap(ledger.summary(for: "qwen", now: night, calendar: calendar))
        ...
        let yesterdays = try XCTUnwrap(ledger.totals(for: "qwen", on: yesterday, calendar: calendar))
        XCTAssertEqual(yesterdays.requests, 1)
        XCTAssertEqual(yesterdays.inputTokens, 1000)
        let all = ledger.totalsToday(now: night, calendar: calendar)
        ...
        let tomorrow = try XCTUnwrap(ledger.summary(for: "qwen", now: night.addingTimeInterval(60), calendar: calendar))
    ...
    func testTheNewestResponseKeepsTheCellWhateverOrderLinesArrive() {
        ...
        var ledger = LocalTokenLedger()
        ledger.record(prediction(at: later, input: 2), calendar: calendar)
        ledger.record(prediction(at: earlier, input: 1), calendar: calendar)
        XCTAssertEqual(ledger.last["qwen"]?.inputTokens, 2)
        ledger.record(prediction(at: later, input: 3), as: "lmstudio:model:qwen", calendar: calendar)
    ...
    func testOldDaysAgeOut() throws {
        ...
        var ledger = LocalTokenLedger()
        ledger.record(prediction(at: ancient), calendar: calendar)
        ledger.record(prediction(at: recent), calendar: calendar)
        ledger.record(prediction(at: today), calendar: calendar)
        XCTAssertNil(ledger.totals(for: "qwen", on: ancient, calendar: calendar))
        XCTAssertNotNil(ledger.totals(for: "qwen", on: recent, calendar: calendar))
